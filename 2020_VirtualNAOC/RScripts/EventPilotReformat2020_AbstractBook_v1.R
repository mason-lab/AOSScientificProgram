...
### Read in xlsx ###
talks<-read.xlsx2("~/Desktop/DesktopClutter25Oct/Service/AOS/AOS_ScientificProgramCommittee/2020/EventPilot 08_01_20.xlsx",stringsAsFactors=F,sheetName="Agenda")
speakers<-read.xlsx2("~/Desktop/DesktopClutter25Oct/Service/AOS/AOS_ScientificProgramCommittee/2020/EventPilot 08_01_20.xlsx",stringsAsFactors=F,sheetName="Speakers")
...
talks_trim<-talks[178:1084,]

talks_trim$Abstract<-sapply(strsplit(talks_trim$Session.Description, "Abstract: "),function(x) x[2])

### Remove weird characters from Abstracts ###
talks_trim$Abstract<-gsub("","---",talks_trim$Abstract)
talks_trim$Abstract <-gsub("\\n"," ",talks_trim$Abstract)
talks_trim$Abstract <-gsub("&","\\\\&",talks_trim$Abstract)
talks_trim$Abstract <-gsub("’","'",talks_trim$Abstract)
talks_trim$Abstract <-gsub("ʻ","'",talks_trim$Abstract)
talks_trim$Abstract <-gsub("“","\"",talks_trim$Abstract)
talks_trim$Abstract <-gsub("”","\"",talks_trim$Abstract)
talks_trim$Abstract <-gsub("‘","'",talks_trim$Abstract)
talks_trim$Abstract <-gsub("%","\\\\%",talks_trim$Abstract)
...
talks_trim$Abstract <-gsub("<","$<$",talks_trim$Abstract)
talks_trim$Abstract <-gsub(">","$>$",talks_trim$Abstract)
talks_trim$Abstract <-gsub("_","\\\\_",talks_trim$Abstract)
talks_trim$Abstract <-gsub("\\^","\\textasciicircum ",talks_trim$Abstract)
talks_trim$Abstract <-gsub("Ō","\\\\={O}",talks_trim$Abstract)
talks_trim$Abstract <-gsub("ā","\\\\={a}",talks_trim$Abstract)
talks_trim$Abstract <-gsub(" \""," ``",talks_trim$Abstract,ignore.case=F)
talks_trim$Abstract <-gsub(" '"," `",talks_trim$Abstract,ignore.case=F)
talks_trim$Abstract<-gsub("é","\\\\'{e}",talks_trim$Abstract)
talks_trim$Abstract<-gsub("á","\\\\'{a}",talks_trim$Abstract)
talks_trim$Abstract<-gsub("ó","\\\\'{o}",talks_trim$Abstract)
talks_trim$Abstract<-gsub("í","\\\\'{i}",talks_trim$Abstract)
talks_trim$Abstract<-gsub("ñ","\\\\~{n}",talks_trim$Abstract)

### Remove weird characters from Session.Title ###
talks_trim$Session.Title<-gsub("\\n"," ",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("&","\\\\&",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("’","'",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("“","\"",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("”","\"",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("ʻ","'",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("‘","'",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("Ō","\\\\={O}",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("ā","\\\\={a}",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("–","--",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("–","--",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("-","-",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("°","\\\\textdegree ",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("é","\\\\'{e}",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("á","\\\\'{a}",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("ó","\\\\'{o}",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("í","\\\\'{i}",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("ñ","\\\\~{n}",talks_trim$Session.Title)
talks_trim$Session.Title<-gsub("_x000b_","",talks_trim$Session.Title)


#$Title<-gsub("ebird ","{e}Bird",talks$Title,ignore.case=T)

### Identify talk slots to skip for abstract book ###
talks_trim<-talks_trim[! talks_trim$Session.Title=="Questions",]
talks_trim<-talks_trim[! talks_trim$Session.Title=="Panel Discussion",]
talks_trim<-talks_trim[! talks_trim$Session.Title=="Introduction to the Symposium",]
talks_trim$Session.Title[grep("Introduction",talks_trim$Session.Title)]

### remove Golden Auk lightining talks ###
talks_trim<-talks_trim[!(talks_trim$Date=="44058" & talks_trim$Session.Types=="Lightning Talks" & talks_trim$Location=="04"),]

### remove EP minitalk symposium ###
talks_trim<-talks_trim[!(talks_trim$Date=="44058" & talks_trim$Session.Types=="Lightning Talks" & talks_trim$Location=="03"),]

### Fix Author names ###
authors_tmp<-sapply(strsplit(talks_trim$Session.Description, "Authors: "),function(x) x[2])
authors_tmp<-sapply(strsplit(authors_tmp,"\n\n"),function(x) x[1])
authors_tmp<-sapply(strsplit(authors_tmp,", Abstract:"),function(x) x[1])
authors_tmp<-sapply(strsplit(authors_tmp,",  Abstract:"),function(x) x[1])

authors_tmp <-gsub("á","\\\\'{a}", authors_tmp)
authors_tmp <-gsub("ó","\\\\'{o}", authors_tmp)
authors_tmp <-gsub("í","\\\\'{i}", authors_tmp)
authors_tmp <-gsub("é","\\\\'{e}", authors_tmp)
authors_tmp <-gsub("ñ","\\\\~{n}", authors_tmp)

talks_trim$authors<-authors_tmp

### Find student presentation award talks ###
talks_trim$PresentationAwardApplicant<-1:nrow(talks_trim) %in% grep("[*]", talks_trim$Session.Description)

### Separate Posters, lightning talks, regular sessions ###
lightning_talks<-talks_trim[talks_trim$Session.Types=="Lightning Talks",]
posters<-talks_trim[talks_trim$Session.Types=="Poster Session",]
regular_talks<-talks_trim[talks_trim$Session.Types=="Paper Session",]
symposia<-talks_trim[talks_trim$Session.Types=="Symposium",]

### Combine symposia and regular talks since we do not have symposium titles ###
regular_talks<-rbind(regular_talks, symposia)

########################
### ABSTRACT BOOKLET ###
######################## 
## Regular talks first ##
## Sort by lastname, firstname of Speaker.ID field ##
rt_id<-sapply(strsplit(regular_talks$Speaker.ID,","),function(x) x[1])
rt_fn<-vector()
rt_ln<-vector()

for(i in 1:nrow(regular_talks)){
	rt_ln[i]<-speakers$Last.Name[speakers$Speaker.ID == rt_id[i]]
	rt_fn[i]<-speakers$First.Name[speakers$Speaker.ID == rt_id[i]]
}
regular_talks[i,]

regular_talks_ordered<-regular_talks[order(rt_ln,rt_fn),]

sink("~/Desktop/DesktopClutter25Oct/Service/AOS/AOS_ScientificProgramCommittee/2020/AbstractBook/papersessions.tex")
for(i in 1:nrow(regular_talks_ordered)){
	cat("\\normaltalk")
	cat("{", regular_talks_ordered $Session.Title[i],"}",sep="")
	cat("{", regular_talks_ordered $authors[i],"}",sep="")
	cat("{", regular_talks_ordered $Abstract[i],"}",sep="")
	cat("\n\n")
}
sink()

### Lightning Talks ###
lt_id<-sapply(strsplit(lightning_talks$Speaker.ID,","),function(x) x[1])
lt_ln<-vector()
lt_fn<-vector()

for(i in 1:nrow(lightning_talks)){
	lt_ln[i]<-speakers$Last.Name[speakers$Speaker.ID == lt_id[i]]
	lt_fn[i]<-speakers$First.Name[speakers$Speaker.ID == lt_id[i]]
}

lightning_talks_ordered<-lightning_talks[order(lt_ln,lt_fn),]

sink("~/Desktop/DesktopClutter25Oct/Service/AOS/AOS_ScientificProgramCommittee/2020/AbstractBook/lightningtalks.tex")
for(i in 1:nrow(lightning_talks_ordered)){
	cat("\\normaltalk")
	cat("{", lightning_talks_ordered $Session.Title[i],"}",sep="")
	cat("{", lightning_talks_ordered $authors[i],"}",sep="")
	cat("{", lightning_talks_ordered $Abstract[i],"}",sep="")
	cat("\n\n")
}
sink()

### Posters ###
po_id<-sapply(strsplit(posters$Speaker.ID,","),function(x) x[1])
po_ln<-vector()
po_fn<-vector()

for(i in 1:nrow(regular_talks)){
	po_ln[i]<-speakers$Last.Name[speakers$Speaker.ID == po_id[i]]
	po_fn[i]<-speakers $First.Name[speakers$Speaker.ID == po_id[i]]
}

posters_ordered<-posters[order(po_ln,po_fn),]

sink("~/Desktop/DesktopClutter25Oct/Service/AOS/AOS_ScientificProgramCommittee/2020/AbstractBook/posters.tex")
for(i in 1:nrow(posters)){
	cat("\\normaltalk")
	cat("{", posters $Session.Title[i],"}",sep="")
	cat("{", posters $authors[i],"}",sep="")
	cat("{", posters $Abstract[i],"}",sep="")
	cat("\n\n")
}
sink()

# ## END HERE FOR NAOC 2020 ### 
# ## REST IS HOLDOVER FROM 2019 ANCHORAGE MEETING ###
...
# talks_trim$Session.Title
# for(i in 1:length(talks$Session.Title)){
