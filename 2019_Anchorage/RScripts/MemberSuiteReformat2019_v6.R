...
### Read in xlsx ###
talks<-read.xlsx2("~/AOSScientificProgram/2019_Anchorage/AOS 2019 Abstracts_Master_11 June 2019r.xlsx",stringsAsFactors=F,sheetName="Search Results")
			...
			for(m in 1:length(times)){
				...
				if(m==1){
					...
					symptite<-symptite[order(unique(as.numeric(day_time_room_list[[k]]$Room.Name)))]
						...
						symptite<-paste(symptite," \\par ",unique(day_time_room_list[[k]]$Code.for.Each.Session)[unique(as.numeric(day_time_room_list[[k]]$Room.Name))],sep="")
