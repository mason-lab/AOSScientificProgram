...
### Read in xlsx ###
talks<-read.xlsx2("~/AOSScientificProgram/2019_Anchorage/AOS 2019 Abstracts_Master_11 June 2019.xlsx",stringsAsFactors=F,sheetName="Search Results")
	...
	for(j in 1:length(day_time_list[[i]])){
		...
		for(k in 1:length(day_time_room_list)){
			...
			if(any(day_time_room_list[[k]]$Session.Title == "Lightning Talks")){
				...
				light_df$Title<-paste0("\\scriptsize ", light_df$Lightning.Title,"\\par \\tiny ", light_df$"Author 1 Short Name"," et al.")
...
posters$Poster.Number<-paste(sapply(strsplit(posters$Day,""),function(x) x[1]),".",sprintf("%02s",posters$Topical.Poster.Order),sep="")
