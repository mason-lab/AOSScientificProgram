...
talks_man<-read.xlsx("~/Desktop/Service/AOS_ProgramBooklet/Tucson2018/CleanedMSOutput/Tucson2018_Submissions_Cleaned_01Mar2018_NAM.xlsx",sheetName="Sheet1",row.names=1,stringsAsFactors=F)
...
sched<-read.xlsx("Abstracts for Program Book_5 March 2018x",sheetName="Abstracts 2-13-18.txt",stringsAsFactors=F)
...
for(i in 1:nrow(sched)){
	if(is.na(sched[i,]$"Entrant..Individual....ID")){next}else{		
		if(nsubs[which(names(nsubs)==sched[i,]$"Entrant..Individual....ID")]==1){
			sched$Abstract_ID<-rownames(talks_entrant_ID[[which(names(nsubs)==sched[i,]$"Entrant..Individual....ID")]])
		}else{next}
	}
}
